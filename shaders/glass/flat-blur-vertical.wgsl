// Vertical Gaussian blur constrained to one rectangular medium.
...
fn fs_main(@location(0) v_uv: vec2f) -> @location(0) vec4f {
  ...
  for (var i: i32 = 1; i <= blurRadius; i = i + 1) {
    ...
    let offset = f32(i) * texelSize.y;
    let positive = textureSampleLevel(u_source, u_sampler, boundedUv(v_uv + vec2f(0.0, offset)), 0.0);
    let negative = textureSampleLevel(u_source, u_sampler, boundedUv(v_uv - vec2f(0.0, offset)), 0.0);
